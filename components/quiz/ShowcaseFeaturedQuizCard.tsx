...
import { Star, Users, Clock, Zap, Activity, ChevronRight } from "lucide-react";
...
export function ShowcaseFeaturedQuizCard({
  ...
  return (
    <div className={cn("relative group w-full", className)}>
      ...
      <div className="absolute -inset-4 bg-gradient-to-r from-primary/20 via-primary/5 to-secondary/20 blur-3xl opacity-40 group-hover:opacity-60 transition-opacity rounded-none" />

      <div className="relative overflow-hidden rounded-none glass-elevated border border-white/10 flex flex-col md:flex-row transition-all duration-500 group-hover:border-primary/20 shadow-2xl">
        {/* Image Section */}
        <div className="relative aspect-[16/9] md:aspect-auto md:w-2/5 overflow-hidden">
          ...
          <div className="absolute top-8 left-8">
            <div className="flex items-center gap-3 px-4 py-1.5 rounded-none glass border border-white/10 text-[10px] font-black uppercase tracking-[0.3em] shadow-lg">
              <div className="h-1.5 w-1.5 rounded-none bg-primary animate-pulse shadow-neon-cyan" />
              {category.toUpperCase()}
            </div>
          </div>
        </div>

        <div className="flex-1 p-6 md:p-10 lg:p-16 flex flex-col justify-between gap-6 md:gap-10">
          <div className="space-y-6 md:space-y-8">
            <div className="space-y-6">
              <div className="hidden md:flex items-center gap-4">
                <div className="h-10 w-1 rounded-none bg-primary shadow-neon-cyan" />
                <span className="text-xs font-black uppercase tracking-[0.5em] text-primary">MISSION CRITICAL</span>
              </div>
              <h2 className={cn("text-2xl sm:text-3xl lg:text-5xl font-black uppercase tracking-tighter leading-[0.9] group-hover:text-primary transition-colors", getGradientText("neon"))}>
                {title}
              </h2>
              {subtitle && (
            ...
            <div className="grid grid-cols-2 gap-3 md:gap-4">
              {[
                ...
                <div key={stat.label} className="p-3 md:p-4 rounded-none glass border border-white/5 space-y-1 group/stat">
          ...
          {ctaHref && (
            <Button asChild variant="accent" size="xl" className="hidden rounded-none px-12 transition-transform group-hover:scale-[1.05] md:inline-flex md:w-fit">
              <Link href={ctaHref}>
                {ctaLabel}
