...
function extractPageFromQuery(data: any): {
  ...
  if (!page || page.missing !== undefined) {
    throw new Error("No Wikipedia page details found");
  }
...
export async function lookupWikipediaTopicMetadata(query: string): Promise<TopicWikiMetadata> {
  ...
  if (!firstResult || !firstResult.title) {
    throw new Error(`No Wikipedia result found for "${queryText}"`);
  }
