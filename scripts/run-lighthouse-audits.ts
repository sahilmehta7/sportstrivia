 ...
 * 
 * Usage:
 *   tsx scripts/run-lighthouse-audits.ts localhost
 *   tsx scripts/run-lighthouse-audits.ts production
 *   tsx scripts/run-lighthouse-audits.ts all
 */
...
async function runAuditsForEnvironment(environment: 'localhost' | 'production'): Promise<void> {
...
async function main() {
  ...
  if (!environmentArg || !['localhost', 'production', 'all'].includes(environmentArg)) {
    console.error('Usage: tsx scripts/run-lighthouse-audits.ts [localhost|production|all]');
    process.exit(1);
  }
  ...
  if (environmentArg === 'all') {
    ...
    await runAuditsForEnvironment(environmentArg as 'localhost' | 'production');
  }
